...
const logout = async () => {
  await loki.user?.logout() || console.warn('loki.user unavailable; logout message received.')
        ...
        <Menu as="div" class="relative inline-block text-left">
          <div>
            <MenuButton class="group w-full bg-gray-800 px-2 py-2 text-sm text-left font-medium">
              <span class="flex w-full justify-between items-center space-x-2">
          ...
          <Transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <MenuItems class="z-10 mx-3 p-1.5 origin-top absolute right-0 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-200 focus:outline-none">
              <div>
